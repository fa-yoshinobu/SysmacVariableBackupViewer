...
このリリースでは、自動テストの導入とそれに伴う不具合の修正、および静的解析によるコード品質の向上を行いました。

### ハイライト
- xUnit によるユニットテストプロジェクトを導入し、コアロジックの継続的な検証を可能にしました。
- BOOL 型の数値表現（1=True, 0=False）が正しく変換されない不具合を修正しました。

### 変更概要
#### 自動テストと品質向上
- `SysmacXmlViewer.Tests` プロジェクトを追加し、データ型変換および XML 解析のテストケースを実装。
- .NET アナライザーによる静的解析を導入し、パフォーマンスとコードスタイルの潜在的な問題を特定。

#### 不具合修正
- `DataTypeConverter`: BOOL 型の値が "1" または "0" で提供された場合に、それぞれ "True", "False" に変換されるよう修正。
